        ...
        {loaded && rows.length === 0 ? (
          ...
          <table className="data w-full">
            ...
            <tbody>{rows.map((r: any) => (
              <tr key={r.id}>
                <td className="pl-5 font-mono text-[13px] text-slate-500">{String(r.created_at).slice(0, 19).replace('T', ' ')}</td>
                <td className="font-mono text-[13px] font-semibold">{r.action}</td>
                <td>{r.admin || '—'}</td>
                <td className="font-mono text-[13px]">{r.registration_id || '—'}</td>
                <td className="max-w-md truncate pr-5 text-slate-400">{r.details || '—'}</td>
              </tr>
            ))}</tbody>
          </table>
        )}
