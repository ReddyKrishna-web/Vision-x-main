        ...
        {(d?.recent || []).length === 0 ? (
          ...
          <table className="data w-full">
            ...
            <tbody>
              {(d?.recent || []).map((j: any) => (
                <tr key={j.id}>
                  <td className="pl-5 text-ink-muted">{j.id}</td>
                  <td className="font-mono text-[13px]">{j.registration_id}</td>
                  <td className="text-ink-soft">{j.entity_type}</td>
                  ...
                  <td className="text-ink-muted">{String(j.updated_at).slice(0, 19).replace('T', ' ')}</td>
