  ...
  return (
    <div className="enter mx-auto max-w-2xl pt-10">
      ...
      <div className="mt-5 space-y-2">
        {scopes.map((s) => (
          <a key={s.id} href={'/api/admin/export?scope=' + s.id} className="card-flat flex items-center justify-between gap-3 transition hover:border-accent/40">
            <span><span className="text-sm font-semibold">{s.id.charAt(0) + s.id.slice(1).toLowerCase()}</span><br /><span className="text-sm text-ink-soft">{s.desc}</span></span>
